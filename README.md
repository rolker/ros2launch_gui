...
In order for the ui to be able to send events to the launch system, TimerActions are used to periodically send QueryUserInterface events allowing the launch system to regularly check for new ui actions. The poll runs at 10 Hz (5 Hz in debug mode) and each poll schedules the next timer with `cancel_on_shutdown=False`, so launch does not cancel the timer at shutdown — the loop terminates itself instead, by declining to reschedule once the user interface reports `close_requested` or the launch context reports `is_shutdown`. The second condition is what covers a sibling `OnShutdown` handler that raises, which aborts the whole Shutdown dispatch before the user interface's own handler runs. Because the in-flight timer is waited out rather than cancelled, the poll period is also a floor on shutdown latency (~100 ms at 10 Hz).
